...
class DABNet(nn.Module):
    def __init__(self, num_classes=2, band_num=3, block_1=3, block_2=6):
        super().__init__()
        self.init_conv = nn.Sequential(
            Conv(band_num, 32, 3, 2, padding=1, bn_acti=True),
        ...
        self.bn_prelu_1 = BNPReLU(32 + band_num) # why0627 x+3 全改x+4

        # DAB Block 1
        self.downsample_1 = DownSamplingBlock(32 + band_num, 64)
        self.DAB_Block_1 = nn.Sequential()
        for i in range(0, block_1):
            self.DAB_Block_1.add_module("DAB_Module_1_" + str(i), DABModule(64, d=2))
        self.bn_prelu_2 = BNPReLU(128 + band_num)

        # DAB Block 2
        dilation_block_2 = [3, 3, 7, 7, 13, 13]
        self.downsample_2 = DownSamplingBlock(128 + band_num, 128)
        self.DAB_Block_2 = nn.Sequential()
        for i in range(0, block_2):
            self.DAB_Block_2.add_module("DAB_Module_2_" + str(i),
                                        DABModule(128, d=dilation_block_2[i]))
        self.bn_prelu_3 = BNPReLU(256 + band_num)

        self.classifier = nn.Sequential(Conv(256+band_num, num_classes, 1, 1, padding=0)) 
