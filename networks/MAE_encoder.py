...
from networks.MAE_pos_embed import get_2d_sincos_pos_embed
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder_finetune(self, x):
        # embed patches
        x = self.patch_embed(x)
...
def mae_vit_base_patch16_dec512d8b_populus(**kwargs):
    model = MaskedAutoencoderViT(
        img_size=256,
        in_chans=4,
        patch_size=16,
...
def mae_vit_base_patch16_dec512d8b_populus_small(**kwargs):
    model = MaskedAutoencoderViT(
        img_size=256,
        in_chans=4,
        patch_size=16,
