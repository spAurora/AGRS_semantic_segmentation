...
from ..dtcwt.coeffs import level1 as _level1
from ..dtcwt.coeffs import qshift as _qshift
from ..dwt.lowlevel import afb2d, mypad, prep_filt_afb2d, roll
from ..dwt.lowlevel import prep_filt_sfb2d_nonsep as prep_filt_sfb2d
from ..dwt.lowlevel import sfb2d_nonsep as sfb2d
...
def icplxdual2D(yl, yh, level1="farras", qshift="qshift_a", mode="periodization"):
    ...
    Faf = (
        (
            prep_filt_sfb2d(g0a1, g1a1, g0a1, g1a1, device=dev),
    ...
    af = (
        (
            prep_filt_sfb2d(g0a, g1a, g0a, g1a, device=dev),
    ...
    J = len(yh)
    w = [
        [[[None for i in range(3)] for j in range(2)] for k in range(2)]
        for l in range(J)
    ]
    for j in range(J):
        w[j][0][0][0], w[j][1][1][0] = pm(
            yh[j][:, 2, :, :, :, 0], yh[j][:, 3, :, :, :, 1]
        )
        w[j][0][1][0], w[j][1][0][0] = pm(
            yh[j][:, 3, :, :, :, 0], yh[j][:, 2, :, :, :, 1]
        )
        w[j][0][0][1], w[j][1][1][1] = pm(
            yh[j][:, 0, :, :, :, 0], yh[j][:, 5, :, :, :, 1]
        )
        w[j][0][1][1], w[j][1][0][1] = pm(
            yh[j][:, 5, :, :, :, 0], yh[j][:, 0, :, :, :, 1]
        )
        w[j][0][0][2], w[j][1][1][2] = pm(
            yh[j][:, 1, :, :, :, 0], yh[j][:, 4, :, :, :, 1]
        )
        w[j][0][1][2], w[j][1][0][2] = pm(
            yh[j][:, 4, :, :, :, 0], yh[j][:, 1, :, :, :, 1]
        )
