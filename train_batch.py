...
for k1 in range(1, 6):
    for k2 in range(1, 11):
        if k1 == 1 and k2 <5:
            continue
        '''参数设置'''
        trainListRoot = r'E:\xinjiang_huyang_hongliu\Huyang_test_0808\2-trainlist\5-trainlist_clear_mix_sim_haze_ATSC+convw+perlin_LV2_rate_0.5_' + str(k1) + '_230502.txt' # 训练样本列表
        save_model_path = r'E:\xinjiang_huyang_hongliu\Huyang_test_0808\3-weights' # 训练模型保存路径  
        model = UNet # 选择的训练模型
        save_model_name = '5-UNet-clear_mix_sim_haze_ATSC+convw+perlin_LV2_rate_0.5_' + str(k1) + '-' + str(k2) +'_230502.th' # 训练模型保存名
